...
- **Предпросмотр диалога** транскриптом в стиле Claude Code: реплики (`>` / `⏺`),
  ...
  свёрнут (`… +N lines`), `Enter` раскрывает всё. Плюс поиск по тексту (`/`, переход
  `n` / `N` с подсветкой совпадений).
...
| клик по свёрнутой строке | раскрыть / свернуть (вывод, план, содержимое файла) |
| `Ctrl+o`, `Enter` | раскрыть весь свёрнутый вывод (повторно — свернуть) |
