...
- Показывает **незакоммиченные правки** рабочего дерева (vs `HEAD`), включая untracked.
...
Перезагрузить конфиг: `Cmd+Ctrl+,` (или перезапустить kitty). Открыть: `cmd+shift+r`.
...
**Мышь**: клик по файлу в дереве — выбрать; клик по папке — выбрать, повторный —
свернуть/развернуть; клик по строке диффа — поставить курсор,
...
Список файлов — `git status` (плюс untracked); дифф каждого файла — версия из
`HEAD` («до») против файла на диске («после»). Отвечает на вопрос «что я наменял
с последнего коммита».

Для untracked-файлов и репозитория без коммитов «до» — пусто: весь файл
показывается как добавленный.
