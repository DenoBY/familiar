...
| `test_lsp_completion.py` | parsing completion answers `modules.lsp.completion` on live intelephense/pyright items: lists, arrays and null, `itemDefaults`, insert/replace ranges in UTF-16, the edit an item makes (text typed after the request, the tail after the caret, doubled call parens, re-indented multi-line snippets), auto-import edits and overlaps |
| `test_lsp_snippet.py` | the LSP snippet grammar `modules.lsp.snippet`: tabstops, nested placeholders, choices, mirrors, escapes, variables, PHP `$this`, broken snippets |
| `test_lsp_signature.py` | signature help answers `modules.lsp.signature`: label offsets in UTF-16 and string labels, the signature-level active parameter winning |
| `test_vcs_fuzzy.py` | the completion matcher `modules.vcs.fuzzy`: exact/prefix/fuzzy tiers, the first letter on a word start, camel humps and consecutive letters over scattered ones, match positions, speed on 5000 items, words of the file nearest the caret |
| `test_vcs_tabstops.py` | snippet placeholders `modules.vcs.tabstops`: typing in the current one keeps the others in place, lines inserted above and a split line, an edit over a placeholder ending the session |
| `test_vcs_popup.py` | the editor popups `modules.vcs.popup`: the list under the word and flipping above, clamping to the code pane, the description giving way to the label, the scroll thumb, struck-through deprecated items, flattened markdown documentation beside the list, the signature line keeping the active parameter visible |
| `test_review_complete.py` | autocomplete in the editor: when the list opens (two letters, trigger characters, `⌃Space`/NUL/`⌥Esc`) and when not (Cyrillic, paste, `i`), refiltering and asking again for incomplete lists, `Enter`/`Tab`/`Esc`/arrows, auto-import on top with the caret and one undo, snippet placeholders with `Tab`, late but compatible answers, one request in flight, a worker crash, words of the file without a server, the popup on screen and mouse, the signature hint |
