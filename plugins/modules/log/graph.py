"""Граф веток (одна строка на коммит), как в tig/lazygit,
но с горизонтальными связями.

Раскладка DAG по лейнам из хешей родителей. Два принципа для
«IDE-вида»:
- основная ветка (цепочка первых родителей от HEAD) держится
  на лейне 0 — ствол слева, ветки отходят вбок (иначе крайний
  лейн достаётся просто самому свежему коммиту);
- каждый лейн занимает 2 колонки (узел + связь), поэтому
  мержи/ответвления рисуются горизонтальными пробегами `●─╮`,
  `├─╯`, а не тесными `╮╯`.

Чистая функция без TUI — цвет по номеру ветки выбирает
вызывающий.
"""
...
def _free_slot(lanes: 'list[str | None]', colors: 'list[int]', allow_zero: bool) -> int:
    """Индекс свободного лейна. allow_zero=False резервирует
    лейн 0 под основную ветку.
    """
...
def _main_chain(commits: 'list[dict]') -> 'set[str]':
    """Множество sha основной ветки: цепочка первых родителей
    от коммита с HEAD (иначе от самого свежего). Эти коммиты
    держим на лейне 0.
    """
...
def build_graph(commits: 'list[dict]') -> 'list[dict]':
    ...
    for c in commits:
        ...
        # горизонтальные связи: сходящиеся ветки (here) и
        # открытые merge-лейны (opened)
