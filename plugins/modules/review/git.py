...
def git_changes(root: str) -> list[dict]:
    ...
    for it in items:
        if it['untracked']:
            # noise-каталоги (venv, node_modules…) не читаем: их может быть тысячи,
            # а в дереве они по умолчанию скрыты — статистика не нужна.
...
def detect_base(root: str) -> str:
    ...
    if out:
        # срезаем весь префикс, а не последний сегмент: default-ветка может быть
        # со слэшем (refs/remotes/origin/release/1.0 → release/1.0)
...
def merge_base(root: str, base: str) -> str:
    """Точка расхождения ветки с base; сам base, если merge-base не вычислился."""
...
def scan_changes(root: str, scope: str, base: str) -> list[dict]:
    ...
    if scope == 'staged':
        # явная ревизия: в репозитории без коммитов индекс сравнивается с пустым
        # деревом (голый --cached там поддержан не всеми версиями git)
        ref = 'HEAD' if has_head(root) else EMPTY_TREE
        items = diff_name_status(root, '--cached', ref)
        stats = git_numstat(root, '--cached', ref)
    else:
        # merge-base, а не сам base: two-dot diff против ушедшего вперёд base
        # показал бы чужие коммиты base как «обратные» изменения ветки
