...
from modules.vcs.git import git_root
from modules.vcs.view import DiffTreeView
from modules.log.git import (
    commit_contents, commit_detail, commit_files, fetch, first_parent, load_commits,
    unpushed_shas,
)
...
def _display_refs(refs: list) -> list:
...
class CommitLogHandler(DiffTreeView):
    ...
    def reload_commits(self):
        ...
        self.status = '' if self.all_commits else 'no commits'
    ...
    def do_fetch(self):
        """Подтянуть изменения с удалёнок (git fetch) и перечитать список."""
        if not self.root:
            return
        self.flash = 'fetching…'
        self.draw_screen()
    ...
    def _commit_row(self, c, width, selected):
        ...
        refs = _display_refs(c.get('refs') or [])
    ...
    def _draw_diff_header(self):
        ...
        c = self.commit
        badge = '⑂ ' if c.get('merge') else ''
        refs = ''.join(f'{name} ' for name, _ in _display_refs(c.get('refs') or []))
